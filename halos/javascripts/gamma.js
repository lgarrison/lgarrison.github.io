...
var Gamma = (function() {
	...
	var $window = $( window ),
			...
			if( isSingleview ) {
				...
				if( $item.length ) {
					...
					if( Gamma.svImage ) {
						...
						//_applyAnimation( Gamma.svImage, { opacity : 0 }, Gamma.settings.svImageTransitionSpeedFade, Gamma.supportTransitions, next_img);
						Gamma.svImage.applyStyle({opacity:0});
						next_img();
		...
		_navigate = function( dir ) {
			...
			if( !Gamma.isSV || Gamma.isAnimating || Gamma.isRendering) {
		...
		_singleviewitem = function( $item, anim ) {
			...
			setTimeout( function() {
				...
				_applyAnimation( Gamma.overlay, { 'opacity' : Gamma.settings.svOpacity }, Gamma.settings.speed, Gamma.supportTransitions || !anim, function() {
		...
		_loadSVItemFromGrid = function( data, position, src ) {
			...
			Gamma.settings.svCallback(data);
		},
		...
		_closesingleview = function() {
			...
			if( Gamma.isAnimating || Gamma.isRendering || Gamma.fly ) {
		...
		_prepareSlideshow = function() {
			...
			if( (Gamma.isAnimating && !Gamma.slideshow) || Gamma.isRendering) {
				return false;
			}
			Gamma.isAnimating = true;
		...
		_stopSlideshow = function( pause ) {
			...
			if( Gamma.isAnimating || Gamma.isRendering ) {
				return false;
			}
			Gamma.isAnimating = true;
			...
			switch( type ) {
				...
				case 'singleviewnavigation' : 
					...
					Gamma.svplay.on( 'click.gamma', function() {
						...
						else if( !Gamma.isAnimating && !Gamma.isRendering ) {
